...
### Warum eine Folge trägt — die Lehre aus EP03

Von EP03 gibt es zwei Fassungen mit **demselben Thema, demselben Material und
derselben Stimme**. Die erste war korrekt und langweilig, die zweite trägt.
Der Unterschied lag in fünf Entscheidungen, und die sind übertragbar — nicht
als Schablone, sondern als Fragen, die man an jedes Drehbuch stellt.

**1. Der Hook beginnt bei einem Menschen in einem Widerspruch, nicht bei
einem Phänomen.**

> V1: „Du sitzt in einem Kellerraum an einer der teuersten Universitäten der
> Welt. Vor dir steht eine graue Kiste."
>
> V2: „Der Dekan der Ingenieurfakultät in Princeton hat ein Problem. Er sitzt
> im Keller seines eigenen Gebäudes und versucht, eine graue Kiste mit dem
> Gedanken zu bewegen. Es klappt. Eins von zehntausend Mal."

Beide beschreiben dieselbe Sekunde. Die zweite hat einen Namen, einen Rang und
einen Widerspruch: Wer so weit oben steht, tut so etwas nicht. **Der
Widerspruch ist der Hook**, nicht das Kuriosum.

Das anonyme „Du sitzt…" wirkt beim Schreiben stark, weil es unmittelbar ist.
Es kostet aber genau das, was hält: jemanden, dem etwas passiert.

**2. Die Frage „Wie war das überhaupt möglich?" wird früh beantwortet.**

Jede Folge dieses Kanals löst beim Zuschauer dieselbe stille Frage aus: Wieso
durfte das stattfinden, und wer hat es bezahlt? V1 ließ sie offen, V2
beantwortet sie in Akt 2 — der Gründer von McDonnell Douglas zahlte, weil er
fürchtete, Pilotengedanken könnten die Elektronik seiner Kampfflugzeuge
stören.

Eine unbeantwortete Nebenfrage nagt und lenkt ab. **Wer bezahlt, wer erlaubt,
wer schaut weg — das gehört in den ersten Drittel.**

**3. Zwei Wendungen, nicht eine.**

V1 hatte einen Höhepunkt: die gescheiterte Replikation. Danach lief die Folge
aus. V2 setzt eine zweite Wendung dahinter, und die ist die stärkere: die
Off-Time-Versuche und der Baseline Bind. Aus „Kann ein Gedanke eine Münze
verschieben?" wird „Ist mein Geist überhaupt jemals still?"

**Die zweite Wendung dreht die Frage, statt sie zu beantworten.** Sie sitzt
bei etwa 65 bis 75 Prozent der Laufzeit — dort, wo eine Folge sonst abfällt,
weil der Zuschauer glaubt, er wisse jetzt, worum es geht.

**4. Beide Seiten sprechen mit ihren eigenen Worten.**

Nicht referiert, sondern zitiert — im Original, mit Übersetzung darunter, auf
einer eigenen Karte. In EP03 drei davon: die Laborleitung („How do you get
peer review when you don't have peers?") und zwei Physiker, die sich
widersprechen. Eine Zusammenfassung von beidem wäre kürzer und schwächer.

**5. Der Schluss bindet an etwas Konkretes zurück.**

Die offene Frage am Ende hängt nicht in der Luft, sondern an einer Tatsache
aus der Mitte der Folge: „Warum lässt eine Universität wie diese ihn gewähren,
solange er es selbst bezahlt?" — das greift Princetons zwei Bedingungen aus
Akt 2 wieder auf.

---

**Die Gegenprobe, in Zahlen.** Beide Fassungen wurden gemessen:
...
Der Umfang ist fast gleich. Es ist keine Frage der Länge, sondern der
Reihenfolge und der Frage, die offen bleibt.
...
| **Rekonstruktion** | **≤ 65 %**, wenn Archiv da ist | 36 % | 74 % | 62 % |
...
Wo Archivmaterial existiert, wird **vor dem Schnitt gezählt, was da ist**. Bei
EP03 waren es 56 brauchbare Stücke; geplant hatte ich mit 20.
...
### Zwei Fallstricke im Zusammenbau

**Wertebereich.** Bilder aus **JPEG-Quellen** tragen die Vollbereichs-Kennung
(`pc`) und geben sie an den Encoder weiter; der Untertitelfilter `ass` rechnet
zusätzlich über RGB und kommt ebenfalls als Vollbereich zurück. Ergebnis ist
ein Master in `yuvj420p` statt `yuv420p` — und Schwarzwerte, die beim
Abspielen zwischen Archiv- und Rekonstruktionsbildern springen. In einer Folge
mit 30 Prozent Archivanteil fällt das bei jedem zweiten Schnitt auf.

Beim Zusammenschnitt übernimmt die Datei die Eigenschaft des **ersten**
Segments — deshalb bleibt der Fehler unsichtbar, solange die Folge zufällig
mit einem PNG beginnt. Abhilfe an zwei Stellen:

```text
Segment:    …,scale=w=iw:h=ih:in_range=auto:out_range=tv,format=yuv420p
Schlusslauf: ass=…,scale=w=iw:h=ih:in_range=auto:out_range=tv,format=yuv420p
             -pix_fmt yuv420p -color_range tv
```

Die QA prüft das (`h264_yuv420p`); sie hat den Fehler auch gefunden.

**Encoder-Voreinstellung.** `preset slow` beim Schlusslauf ist für einen
Upload-Master verschenkt, weil YouTube ohnehin neu kodiert. `veryfast` bei
gleichem crf: **3:18 statt gut zehn Minuten**, kein sichtbarer Unterschied.
Für die Segmente bleibt `veryfast` ohnehin gesetzt.
...
- [ ] Öffnet der Hook bei einem Menschen in einem Widerspruch? (§ 1)
- [ ] Ist bis zum ersten Drittel klar, wer bezahlt und wer erlaubt hat?
...
**Am Material, bevor die Zeitachse gebaut wird:**

- [ ] Gezählt, wie viele Archivstücke tatsächlich brauchbar sind (§ 3)
- [ ] `spg_neonpruefung.py` — Innenräume ohne Farbbänder
- [ ] Motive mit Menschen: benannter Zustand, nicht nur eine Pose
- [ ] Motive außerhalb des Hauptschauplatzes: ausdrückliche Absage an ihn.
      Sonst schiebt sich die Vorlage in jedes Bild — bei EP03 saß ein Pilot
      im Cockpit mit Laborbank und Schreibtischlampe dahinter.
- [ ] Karten gegen den Sprechtext gelesen. Eine Karte, die dem Gesagten
      widerspricht, entwertet beides.
...
- [ ] `spg_zappelpruefung.py` — Median unter 0,10
- [ ] QA grün, besonders `h264_yuv420p`
- [ ] Endcard gegengesehen — Rechtschreibung, Palette, kein nachgebauter
      YouTube-Knopf

**Nach dem Upload:**

- [ ] Untertitelspur hochgeladen (nicht nur die Automatik)
- [ ] Endscreen und Infokarten im Studio gesetzt — die API kann das nicht,
      siehe `ENDSCREENS_UND_KARTEN.md`
- [ ] Angepinnter Kommentar (`spg_kommentar.py` setzt ihn, Anpinnen von Hand)
- [ ] Kanalplan gelesen, bevor terminiert wird. Zwei Shorts auf dieselbe
      Minute nehmen sich gegenseitig die Auslieferung weg.
