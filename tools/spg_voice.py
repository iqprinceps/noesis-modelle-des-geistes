#!/usr/bin/env python3
"""EP01A Die Spiegel — Voice-Master bauen und ausrichten.

Abgeleitet von `tools/gw_v7_voice.py` (EP02 Gateway V7); die Vorlage bleibt
unveraendert.

Baut aus den acht George-Stems den durchgehenden VO-Master (Vorlauf, Stems auf
-18 LUFS, Pausen dazwischen, Nachlauf), legt ihn zusaetzlich als
`audio/EP01A_voice_-18LUFS.wav` fuer den Producer ab und holt dann das Forced
Alignment gegen die Reinschrift.

Das Alignment laeuft ausdruecklich gegen `07_VOICE_SCRIPT_CLEAN.txt`, nicht
gegen die Sprechtexte: die Reinschrift traegt die richtige Orthografie, und
genau an ihr haengen spaeter Untertitel und Bildanker.
"""
...
MASTER = MDIR / "EP01A_SPIEGEL_VO_MASTER.wav"
PRODUCER_VOICE = PROD / "audio" / "EP01A_voice_-18LUFS.wav"
...
def build_master():
    ...
    total = dur(MASTER)
    PRODUCER_VOICE.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(MASTER, PRODUCER_VOICE)
