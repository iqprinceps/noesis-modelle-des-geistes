#!/usr/bin/env python3
"""Prepare, assemble, align and independently QA the EP13_DE voice.
...
# German narration, same voice and same settings as the English cut. George on
# eleven_multilingual_v2 speaks German, and keeping the settings identical is what
# makes the two language versions sound like one channel. Only the script and the
# language of the blind transcription QA change.
...
EP = ROOT / "08_GERMAN_PRODUCTION" / "EP13_VATIKAN_01"
SCRIPT = EP / "01_SCRIPT" / "VOICE_SCRIPT_DE.txt"
VOICE = EP / "02_VOICE"
SOURCE = VOICE / "SOURCE"
MASTER = VOICE / "MASTER"
RAW = MASTER / "raw_stems"
NORM = MASTER / "normalized_stems"
ALIGN = VOICE / "ALIGNMENT" / "EP13_DE_VO_ALIGNMENT.json"
...
def prepare() -> None:
    ...
    for i, text in enumerate(chunks(), 1):
        stem_id = f"EP13_DE_TAKE_{i:02d}"
        path = SOURCE / f"{stem_id}.txt"
        path.write_text(text + "\n", encoding="utf-8")
        stems.append({"id": stem_id, "text_file": str(path.resolve())})
    reconstructed = "\n\n".join((SOURCE / f"EP13_DE_TAKE_{i:02d}.txt").read_text(encoding="utf-8").strip() for i in range(1, len(stems) + 1))
    expected = "\n\n".join(paragraphs())
    if reconstructed != expected:
        raise RuntimeError("stems do not reconstruct canonical script")
    batch = {
        "episode": "EP13_DE", "profile": "EP05_EN candidate A, reused", "voice": VOICE_ID, "voice_name": "George",
...
def assemble() -> Path:
    ...
    out = MASTER / "EP13_DE_VO_MASTER.wav"
...
def load_key() -> str:
    ...
    # The shared CLI resolves the key from whichever account profile is active.
    # NOESIS_ELEVEN_PROFILE picks a different one for this process only, without
    # touching the machine-wide setting, which matters when the active account has
    # run out of credits mid-production.
...
def align() -> Path:
    audio = MASTER / "EP13_DE_VO_MASTER.wav"
    text = "\n\n".join(paragraphs())
    ...
    data.update({"episode": "EP13_DE", "profile": "EP05_A", "source_text": text, "audio": str(audio.resolve()), "audio_sha256": sha(audio)})
...
def qa() -> None:
    ...
    with audio.open("rb") as stream:
        response = client.speech_to_text.convert(model_id="scribe_v2", file=stream, language_code="de", tag_audio_events=False, diarize=False, timestamps_granularity="word", seed=260827, keyterms=["Fatima", "Lucia dos Santos", "Agca", "Sodano", "Johannes Paul", "Heiliges Offizium", "Petersplatz", "Napoleon", "Rosenkranz"])
    ...
    out = VOICE / "QA" / "EP13_DE_SCRIBE_CONTENT_QA.json"
