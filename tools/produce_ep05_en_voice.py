#!/usr/bin/env python3
"""Prepare, assemble, align and independently QA the selected EP05_EN voice."""
...
EP = ROOT / "07_ENGLISH_PRODUCTION" / "EP05_SLEEP_PARALYSIS_01"
...
ALIGN = VOICE / "ALIGNMENT" / "EP05_EN_VO_ALIGNMENT.json"
...
def prepare() -> None:
    ...
    for i, text in enumerate(chunks(), 1):
        stem_id = f"EP05_EN_TAKE_{i:02d}"
        path = SOURCE / f"{stem_id}.txt"
        path.write_text(text + "\n", encoding="utf-8")
        stems.append({"id": stem_id, "text_file": str(path.resolve())})
    reconstructed = "\n\n".join((SOURCE / f"EP05_EN_TAKE_{i:02d}.txt").read_text(encoding="utf-8").strip() for i in range(1, len(stems) + 1))
    expected = "\n\n".join(paragraphs())
    if reconstructed != expected:
        raise RuntimeError("stems do not reconstruct canonical script")
    batch = {
        "episode": "EP05_EN", "selected_from_audition": "A", "voice": VOICE_ID, "voice_name": "George",
...
def assemble() -> Path:
    ...
    out = MASTER / "EP05_EN_VO_MASTER.wav"
    run(["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-f", "concat", "-safe", "0", "-i", str(concat_path), "-c:a", "pcm_s24le", str(out)])
    data = {"candidate": "A", "voice": "George", "model": batch["model"], "settings": batch["settings"], "duration": round(duration(out), 3), "loudness": loudness(out), "sha256": sha(out), "stems": report}
...
def multipart(audio: Path, text: str) -> tuple[bytes, str]:
    boundary = "----EP05EN" + uuid.uuid4().hex
...
def align() -> Path:
    audio = MASTER / "EP05_EN_VO_MASTER.wav"
    text = "\n\n".join(paragraphs())
    ...
    data.update({"episode": "EP05_EN", "candidate": "A", "source_text": text, "audio": str(audio.resolve()), "audio_sha256": sha(audio)})
...
def qa() -> None:
    ...
    with audio.open("rb") as stream:
        response = client.speech_to_text.convert(model_id="scribe_v2", file=stream, language_code="en", tag_audio_events=False, diarize=False, timestamps_granularity="word", seed=260827, keyterms=["Hufford", "Fogo Island", "Cheyne", "Takeuchi", "REM atonia", "Intruder", "Incubus", "Bridget Bishop"])
    ...
    out = VOICE / "QA" / "EP05_EN_SCRIBE_CONTENT_QA.json"
