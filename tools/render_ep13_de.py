...
# Pictures, clips and acquired originals are shared with the English cut: the
# German script was written to the same 115 beats, so every frame still belongs
# where it was. Only the cards carry language, and those are German here.
...
CLIPS = ASSET_EP / "03_VISUALS" / "CLIPS"
ORIG = ASSET_EP / "03_VISUALS" / "ASSETS" / "SELECTED" / "AUTHENTIC"
WORK = ROOT / "tmp" / "render" / "ep13_de"
SEGS = WORK / "segments"
OUTFILE = WORK / "EP13_DE_PICTURE.mp4"
...
def resolve(state: str) -> pathlib.Path | None:
    ...
    key = state.upper()
    for candidate in (key, "EP13_" + key, "EP13_DE_" + key):
